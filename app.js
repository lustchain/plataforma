...
async function bridgeFetch(path) {
  ...
  for (const base of [...new Set(urls)]) {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data?.error || `Bridge API ${res.status}`);
      return data;
    } catch (err) {
      lastErr = err;
    }
...
function setRecoverBscStatus(message, tone = "") {
  const el = document.querySelector("[data-bridge-recover-bsc-status]");
  if (!el) return;
  el.textContent = message;
...
function mergeRecoveredClaimIntoList(claim, used = false) {
  if (!claim?.depositId) return;
  const key = String(claim.depositId).toLowerCase();
  const withoutDuplicate = pendingBridgeClaims.filter(({ claim: c }) => String(c?.depositId || "").toLowerCase() !== key);
  pendingBridgeClaims = [{ claim, used }, ...withoutDuplicate].slice(0, 30);
  renderPendingClaims(pendingBridgeClaims);
}

async function recoverBscClaimByHash(txHashInput = "", options = {}) {
  const input = document.querySelector("[data-bridge-recover-bsc-tx]");
  const txHash = String(txHashInput || input?.value || "").trim();

  if (!isHexTxHash(txHash)) {
    setRecoverBscStatus("Enter a valid BSC transaction hash starting with 0x.", "bad");
    if (!options.silent) bridgeLog("Enter a valid BSC transaction hash.", "warn");
    return null;
  }

  if (input && !input.value) input.value = txHash;

  try {
    setRecoverBscStatus("Checking BSC receipt and bridge signatures...", "warn");
    if (!options.silent) bridgeLog("Recovering BSC claim from transaction hash...", "");

    const data = await bridgeFetch(`/api/recover/bsc/${encodeURIComponent(txHash)}`);
    if (!data?.ok) throw new Error(data?.error || "recover_failed");

    const claim = data.claim;
    if (!claim?.depositId) throw new Error("Recovery returned no claim data.");

    localStorage.setItem("lustLastBscDepositTx", txHash);
    localStorage.setItem("lustLastDepositId", claim.depositId);

    const used = Boolean(claim.used) || claim.status === "minted" || await isMintedOnChain(claim.depositId);
    mergeRecoveredClaimIntoList(claim, used);

    const amountText = formatUnitsSafe(claim.amount);
    const sourceLabel = claimSourceLabel(claim);

    if (used) {
      setText("[data-bridge-claim-state]", "Already minted");
      setBridgeActionReady("[data-bridge-mint]", false);
      setRecoverBscStatus(`Recovered ${sourceLabel} claim, but it is already minted.`, "ok");
      if (!options.silent) bridgeLog(`This ${sourceLabel} claim for ${amountText} LUSDT is already minted.`, "ok");
      return claim;
    }

    if (walletState.connected && walletState.address && !sameAddress(claim.recipient, walletState.address)) {
      setRecoverBscStatus("Claim found, but it belongs to a different wallet. Connect the recipient wallet to mint.", "bad");
      bridgeLog("Claim found, but it belongs to a different wallet. Connect the recipient wallet to mint.", "warn");
      return claim;
    }

    await prepareActiveClaim(claim, "ok");
    setRecoverBscStatus(`Recovered BSC claim ready: ${amountText} LUSDT. Switch to LUST Chain and mint.`, "ok");
    return claim;
  } catch (err) {
    console.error(err);
    const msg = err?.message || "Could not recover BSC claim.";
    setRecoverBscStatus(msg, "bad");
    if (!options.silent) bridgeLog(`BSC recovery failed: ${msg}`, "warn");
    return null;
  }
}
...
async function depositToLusdt() {
  try {
    ...
    if (kind === "bsc") {
      localStorage.setItem("lustLastBscDepositTx", tx.hash);
      const recoverInput = document.querySelector("[data-bridge-recover-bsc-tx]");
      if (recoverInput) recoverInput.value = tx.hash;
    }
    ...
    if (kind === "bsc") {
      setRecoverBscStatus("BSC deposit confirmed. Auto-recovery is checking the claim by transaction hash...", "warn");
      setTimeout(() => recoverBscClaimByHash(tx.hash, { silent: true }).catch(() => {}), 2500);
      setTimeout(() => recoverBscClaimByHash(tx.hash, { silent: true }).catch(() => {}), 9000);
    }
  } catch (err) {
    console.error(err);
...
function wireLusdtBridge() {
  ...
  document.querySelector("[data-bridge-recover-bsc]")?.addEventListener("click", () => recoverBscClaimByHash());
  document.querySelector("[data-bridge-recover-bsc-tx]")?.addEventListener("keydown", (event) => {
    if (event.key === "Enter") {
      event.preventDefault();
      recoverBscClaimByHash();
    }
  });
  const lastBscTx = localStorage.getItem("lustLastBscDepositTx") || "";
  const recoverInput = document.querySelector("[data-bridge-recover-bsc-tx]");
  if (recoverInput && lastBscTx && !recoverInput.value) recoverInput.value = lastBscTx;
